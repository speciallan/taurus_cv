#!/usr/bin/env python
# -*- coding:utf-8 -*-
# clone from https://github.com/ming71/toolbox/blob/master/converter/voc2coco.py
...
class PascalVOC2coco(object):
            ...
            with open(json_file, 'r') as fp:
                ...
                for p in fp:
                    ...
                    if 'filename' in p:
                        ...
                        self.path = os.path.join(path, 'SegmentationObject', self.filen_ame.split('.')[
                            0] + '.png')  # if self.path not in obj_path:  #    break

                    if 'width' in p:
                        self.width = int(p.split('>')[1].split('<')[0])  # print(self.width)
                    if 'height' in p:
                        self.height = int(p.split('>')[1].split('<')[0])

                        self.images.append(self.image())  # print(self.image())
    ...
    def mask2polygons(self):
        ...
        for cont in contours[1]:
            [bbox.append(i) for i in list(cont.flatten())]  # map(bbox.append,list(cont.flatten()))
