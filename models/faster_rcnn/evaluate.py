...
from .utils import np_utils, eval_utils, generator
...
def evaluate(args, image_num=200):
    ...
    # train_image_info_list = [info for info in dataset.get_image_info_list() if info['type'] == 'trainval']
    print("len:{}".format(len(test_image_info_list)))

    # 加载模型
    model = network.frcnn(config, stage='test')
    ...
    # 逐个图像处理
    print('test images total:', len(test_image_info_list))

    # 一次最多只能跑200张
    test_image_info_list = test_image_info_list[:image_num]

    # 通过测试集验证模型
    # 内存不够，要使用生成器载入，一次100-200张
    for id in range(len(test_image_info_list)):
        image, image_meta, _ = image_utils.load_image_gt(id, test_image_info_list[id]['filepath'], config.IMAGE_MAX_DIM, test_image_info_list[id]['boxes'])
